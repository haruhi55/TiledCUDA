...
void run_test() {
    ...
    using SwizzledLayout =
        tl::Swizzled<tl::RowMajor<kShmRows, kShmCols>, 2, 3, 3>;
    using Shared2 = SharedTile<Element, SwizzledLayout>;
    ...
    // SharedLayout layout1;
    // SwizzledLayout layout2;

    // std::ostringstream ss;
    // for (int i = 0; i < kRows; ++i) {
    //     for (int j = 0; j < kCols; ++j) {
    //         if (j % 8 == 0) ss << layout1(i, j) / 8 << "|";
    //     }
    //     ss << std::endl;
    // }
    // LOG(INFO) << "non-swizzled layout:" << std::endl << ss.str() <<
    // std::endl; ss.str("");

    // for (int i = 0; i < kRows; ++i) {
    //     for (int j = 0; j < kCols; ++j) {
    //         if (j % 8 == 0) ss << layout2(i, j) / 8 << "|";
    //     }
    //     ss << std::endl;
    // }
    // LOG(INFO) << "swizzled layout:" << std::endl << ss.str() << std::endl;
...
TEST(TestSwizzledLayout, test1) {
    // the minimal shape for load a single BaseTile.
